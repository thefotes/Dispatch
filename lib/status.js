...
const DEFAULTS = {
  ...
  colors: {
    blocked: "#FF2D2D",
    working: "#FFA000",
    // Herdr reports "done" for an agent that has finished and that you have
    // not looked at yet, and "idle" once it is focused. That is a real
    // difference - one is waiting to be read, the other is just quiet - so
  ...
  // per-key colour; the cost is that those keys stop sending keystrokes.
  // Set false to leave the keymap alone and only warn when it is wrong.
  manage_keymap: true,
  poll_ms: 2500,
  debounce_ms: 100,
};
