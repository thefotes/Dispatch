...
class WLDevice {
  ...
  _dispatch(obj) {
    // The device pushes JSON-RPC notifications (no id) for key presses and
    // joystick movement: `v.oai.hid` and `v.oai.rad`.
    if (obj.method !== undefined && obj.id === undefined) {
      if (this.onNotify) this.onNotify(obj.method, obj.params);
      return;
    }
