...
// index, matching the thread id used for lighting.
function agIndex(name) {
  const match = /^AG(\d+)$/.exec(String(name || ""));
  return match ? parseInt(match[1], 10) : null;
}

// Attach handlers for device-pushed events. Returns a detach function.
//
// `act` is 1 on press and 0 on release, so `onKey` fires for both; filter on
// `pressed` if you only want one edge.
function onNotify(dev, { onKey, onJoystick } = {}) {
  ...
  dev.onNotify = (method, params) => {
    if (method === N_HID && onKey) {
      onKey({
        key: params?.k,
        index: agIndex(params?.k),
        act: params?.act,
        pressed: params?.act === 1,
        agent: params?.ag,
      });
